...
possible: config, spec, credentials, gitignore, and trace receipts. Do
...
- add the smallest optional application injection seam only when the spec
  `.beaker/` cannot reuse an existing interface. Preserve
...
path. First adapt through the spec or helper code under `.beaker/`. Touch

...
| On an SDK older than 0.4.3, local smoke passes but the hosted image reports a missing source/import directory. | Older local path resolution treated `source_dir` as project-relative while hosted builds treated it as Git-root-relative. | Upgrade the SDK, then rerun smoke with the full repository-relative `--config-file` and the nested project's package-manager environment. |

...
| Local smoke passes but the hosted image build reports a missing source/import directory. | Local smoke was run from the nested project root and accidentally validated different path coordinates. | Run smoke from the Git root with the full repository-relative `--config-file` and the nested project's package-manager environment. |

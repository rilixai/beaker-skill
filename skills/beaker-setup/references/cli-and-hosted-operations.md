...
- Agents represent optimization targets, not repositories. Rerunning agent login synchronizes the repository-relative Beaker config path.
...
Both build and trigger default to `.beaker/beaker.yaml`; use `--config-file` or `BEAKER_CONFIG_FILE` for another repository-relative path.
